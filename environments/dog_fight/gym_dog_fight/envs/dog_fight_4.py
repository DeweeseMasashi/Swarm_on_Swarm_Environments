...
class DogFightEnv4(gym.Env):
    ...
    def __init__(self):
        ...
        high = np.array([self.max_distance, self.max_distance, 180, 360])
        ...
        self.curr_step = -1
        self.prev_state = (self.position[0], self.position[1], 0, 0)
    ...
    def is_done(self):
        return self.out_of_bounds or in_radius(self.position[0], self.position[1], self.enemy_position[0], self.enemy_position[1], self.movement_length*10)
    ...
    def _get_reward(self):
        if self.out_of_bounds:
            return -10
        elif in_radius(self.position[0], self.position[1], self.enemy_position[0], self.enemy_position[1], self.movement_length*10):
    ...
    def _get_state(self):
        ...
        if angleInRange(self.getActorAngle(), angle - 45, angle + 45):
            # displaced by 45 degrees to differentiate from 0
            return (self.position[0], self.position[1], (self.getActorAngle() - angle + 90) % 360, self.getActorAngle())
        else:
            return (self.position[0], self.position[1], 0, self.getActorAngle())
        '''
